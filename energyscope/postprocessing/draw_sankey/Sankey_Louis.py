...
def Sankey_plot(df, storage):
    ...
    """
    list_storage = ["BATT_LI", "BEV_BATT", "PHEV_BATT", "PHS", "TS_DEC_DIRECT_ELEC", "TS_DEC_HP_ELEC",
                    ...
                    "TS_DHN_DAILY", "TS_DHN_SEASONAL", "TS_HIGH_TEMP", "H2_STORAGE"]
    ...
    for i in df['Tech']:
        if i != 'END_USES_DEMAND' and 'URANIUM':  # Warning : There is a space before and after the name for the lines
            ...
            for col in dfi2:
                ...
                if col not in ['Tech', 'END_USES_DEMAND', 'CO2_DECENTRALISED', 'CO2_CENTRALISED', 'CO2_CAPTURED',
                               'CO2_ATMOSPHERE', 'URANIUM', 'OTHER_GHG', 'RES_WIND', 'RES_HYDRO', 'RES_SOLAR',
                               'RES_GEO']:
                    if i in list_storage:
                        ...
                        if i.startswith('TS_DEC'):
                            i = 'TS_DEC'
                        SOURCE, TARGET = col, i
                        sankey_df = sankey_df.append({'source': SOURCE, 'target': TARGET, 'realValue': VAL_in},
                                                     ignore_index=True)
                        sankey_df = sankey_df.append({'source': TARGET, 'target': SOURCE, 'realValue': VAL_out},
                                                     ignore_index=True)
                    else:
                        VAL = dfi2[col].item()
                        if col == name_row:
                        ...
                        if abs(VAL) > 500:
                            ...
                            sankey_df = sankey_df.append({'source': SOURCE, 'target': TARGET, 'realValue': VALUE},
                                                         ignore_index=True)
        ...
        if i == 'END_USES_DEMAND':  # plot the end_uses demand differentiated by the vector
            ...
            name_row = dfi2['Tech'].item()
            for col in dfi2:
                if col not in ['Tech', 'MOB_PRIVATE', 'MOB_AVIATION', 'MOB_PUBLIC', 'MOB_FREIGHT_RAIL',
                               'MOB_FREIGHT_ROAD', 'HVC', 'MOB_SHIPPING']:
                    VAL = dfi2[col].item()
    ...
    if EUD_energy_unit == True:
        for EUD in ['MOB_PRIVATE', 'MOB_AVIATION', 'MOB_PUBLIC', 'MOB_FREIGHT_RAIL', 'MOB_FREIGHT_ROAD']:
            ...
            for i in range(0, list_source_EUD.size):
                ...
                if sankey_df[sankey_df['target'] == name_source][
                    'realValue'].size != 0:  # Replace the value with the corresponding energy value
                    sankey_df['realValue'][(sankey_df['target'] == EUD) & (sankey_df['source'] == name_source)] = \
                    sankey_df[sankey_df['target'] == name_source]['realValue'].iloc[0]
    ...
    if change_color == True:
        techno_color = pd.read_excel('/Users/achuat/Documents/Projects/Governance_EnergyScope/Sankey Louis/techno_color.xlsx',
                                     index_col=False)
        ...
        sankey_df_color = [
            make_rgb_transparent(techno_color[techno_color['Name'] == i.replace(' ', '')]['Color_vector'].iloc[0], 0.5)
            for i in ss]
        Sankey = go.Figure(go.Sankey(
            node={'pad': 10, 'thickness': 20, 'label': st_uni, 'color': COLOR_node, 'label': NAME_node},
            link={"source": pos_source, "target": pos_target, "value": sankey_df['realValue'].tolist(),
                  "customdata": sankey_df['realValue'].tolist(), "arrowlen": 20, 'color': sankey_df_color}))
    else:
        Sankey = go.Figure(go.Sankey(
            node={
                'pad': 10,
    ...
    return Sankey
def Sankey_plot_new(amplypy_obj):
    """
    Generates a Sankey plot of the energy flows from a Pandas DataFrame.
    """
    df_flo = amplypy_obj
    ...
    df_flo = df_flo.reset_index()
    print(df_flo)
    sankey_df = pd.DataFrame(columns=['source', 'target', 'realValue'])
    for i in df_flo['Tech']:
        if i != 'END_USES_DEMAND' and 'URANIUM':  # Warning : There is a space before and after the name for the lines
            dfi = df_flo[df_flo['Tech'] == i]
            dfi2 = dfi[dfi != 0].dropna(axis=1)
            name_row = dfi2['Tech'].item()

            for col in dfi2:
                # To make a plot where the df_sto is looping
                if col not in ['Tech', 'END_USES_DEMAND', 'CO2_DECENTRALISED', 'CO2_CENTRALISED', 'CO2_CAPTURED',
                               'CO2_ATMOSPHERE', 'URANIUM', 'OTHER_GHG', 'RES_WIND', 'RES_HYDRO', 'RES_SOLAR',
                               'RES_GEO']:
                    if i in list_storage:
                        VAL_in = df_sto.loc[i][col + '_in']
                        VAL_out = df_sto.loc[i][col + '_out']
                        if (VAL_out) < 500:
                            VAL_in, VAL_out = 0, 0
                        if i.startswith('TS_DEC'):
                            i = 'TS_DEC'
    ...
    print(list(map(str, sankey_df['realValue'].tolist())))
    if change_color == True:
        techno_color = pd.read_excel(os.path.join(os.getcwd(), 'energyscope', 'postprocessing', "techno_color.xlsx"),
                                     index_col=False)
        COLOR_node = [techno_color[techno_color['Name'] == i.replace(' ', '')]['Color_vector'].item() for i in st_uni]
        NAME_node = [techno_color[techno_color['Name'] == i.replace(' ', '')]['Name_sankey'].item() for i in st_uni]
        sankey_df_color = [
...
def Sankey_carbon(df, gwp_construct):
        ...
        if i != 'END_USES_DEMAND':  # Warning : There is a space before and after the name for the lines
            ...
            for col in dfi2:
                ...
                if col in ['CO2_DECENTRALISED', 'CO2_CENTRALISED', 'CO2_CAPTURED', 'CO2_ATMOSPHERE', 'OTHER_GHG']:
                    ...
                    if (name_row == 'GHG_EMISSIONS' and col == 'OTHER_GHG') or (
                            name_row == 'CO2_EMISSIONS' and col == 'CO2_DECENTRALISED') or (
                            name_row == 'CO2_ATMOSPHERE_IMP' and col == 'CO2_CENTRALISED'):
                        name_row = 'CO2_ATMOSPHERE'
                    if (name_row == 'GHG_EMISSIONS' and col == 'CO2_ATMOSPHERE') or (
                            name_row == 'CO2_EMISSIONS' and col == 'CO2_ATMOSPHERE') or (
                            name_row == 'CO2_ATMOSPHERE_IMP' and col == 'CO2_ATMOSPHERE'):
                        VAL = 0
                ...
                if col in ['GAS', 'JETFUEL', 'WOOD', 'DIESEL', 'GASOLINE', 'LFO', 'COAL', 'WET_BIOMASS', 'METHANOL',
                           'WASTE', 'CEMENT_PROCESS', 'FT_FUEL', 'HVC']:
                    if col == 'HVC':
    ...
    sankey_df = sankey_df.append(
        {'source': 'GHG_CONSTRUCTION', 'target': 'CO2_ATMOSPHERE', 'realValue': gwp_construct['GWP_constr'].sum()},
        ignore_index=True)
    ...
    if change_color == True:
        ...
        NAME_node = [techno_color[techno_color['Name'] == i.replace(' ', '')]['Name_sankey'].item() for i in st_uni]
        sankey_df_color = [
            make_rgb_transparent(techno_color[techno_color['Name'] == i.replace(' ', '')]['Color_carbon'].item(), 0.5)
            for i in tt]
        Sankey = go.Figure(go.Sankey(
            node={'pad': 10, 'thickness': 20, 'label': st_uni, 'color': COLOR_node, 'label': NAME_node},
            link={"source": pos_source, "target": pos_target, "value": sankey_df['realValue'].tolist(),
                  "label": sankey_df['realValue'].tolist(), 'color': sankey_df_color, "arrowlen": 15})
        )  # ,visible=True)
    else:
        Sankey = go.Figure(go.Sankey(
            node={'pad': 10, 'thickness': 20, 'label': st_uni, },
            link={"source": pos_source, "target": pos_target, "value": sankey_df['realValue'].tolist(),
                  "label": sankey_df['realValue'].tolist(), "arrowlen": 150}))
